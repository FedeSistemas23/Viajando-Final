    ...
    //
{
    public class CD_Usuario : Conexion
    {
        ...
        {            
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@FechaPrimerIngreso", CS_UsuarioEnSesion.fechaPrimerIngreso);        
            ...
            try
            {
                if (leer.HasRows)
                {
                    while (leer.Read()) 
                    {
                        ...
                        CS_UsuarioEnSesion.intentos = leer.GetInt32(leer.GetOrdinal("IntentosFallidos"));
